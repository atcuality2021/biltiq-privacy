...
| Logging redaction | `biltiq_privacy.core.log_filter.RedactionFilter` | **PLANNED — not yet implemented** (no `core/log_filter.py` exists; caught in BILTIQ-012 review when the README cited it). `logging.Filter` subclass — scrubs `record.msg`, `record.args`, `extra=` fields. Needs its own ticket before anything references it again. |
...
| Anonymisation facade | `biltiq_privacy.pipeline.anonymise(text, *, detector, key, generated_at, regime=None, generalise=True, prev_hash=GENESIS_PREV_HASH)` → frozen `AnonymiseResult` (BILTIQ-012) | The public entry point — use it instead of hand-chaining detector → pseudonymiser → generaliser → regime → chain. Caller-supplied `generated_at`/`prev_hash` (no clock read); PII-free chain payload (counts/flags/SHA-256 commitments); `_merge_detections` dedup is higher-score-wins (CDSCO policy — intentionally distinct from the planned BILTIQ-015 cross-backend CompositeDetector). Re-exported from the `biltiq_privacy` top level alongside the 18-name v0.1.0 surface. |
...
| Default detector | `biltiq_privacy.detectors.presidio_backend.PresidioDetector(*, score_threshold=0.5, auto_download_model=False)` (BILTIQ-009, BILTIQ-012) | Ports CDSCO's `detect_pii_rules()` behind the ABC, reusing `indian.recognisers.build_engine()` (no recogniser re-defined). Instance-level lazy singleton (engine built on first `detect()`); configurable threshold; six-key `DetectedEntity` projection with `source="presidio"` + `round(score,4)`. Presidio/spaCy imports are method-level (AC5); a missing `en_core_web_sm` raises `MissingNERModelError` naming both remedies (post-install or `auto_download_model=True` — explicit opt-in network call, default OFF; ADR-0006). Re-exported from `biltiq_privacy.detectors`. |
| Regime ABC + report contract | `biltiq_privacy.regimes.base.Regime` / `ComplianceCheck` / `ComplianceReport` (BILTIQ-011) | `validate(original, anonymised, detections, audit_records, *, generated_at) -> ComplianceReport` — implement to add a regulatory framework (GDPR/HIPAA/CCPA in Phase C). Report types are `@dataclass(slots=True, frozen=True)` with `status: Literal["pass","fail","warning","info"]`; `compliant` requires all checks `"pass"`. `generated_at` is caller-supplied (no clock read — BILTIQ-010 determinism precedent). Consumes the 007 `Detection`/`AuditRecord` TypedDicts; a 009 `DetectedEntity` flows in unchanged. |
| DPDP 2023 regime | `biltiq_privacy.regimes.dpdp.DPDPRegime(*, generalisation_markers=("XXXX", "Healthcare Facility", "India"), include_values=False)` (BILTIQ-011) | The eight CDSCO checks (`DPDP-1..6`, `NDHM-1`, `ICMR-1`) ported behaviour-equivalent modulo four documented deltas (each test-pinned): stricter 9-type residual scan reusing BILTIQ-002 `PATTERNS` + ported `EMAIL_PATTERN`; `\b` phone boundary; locators-only evidence by default (`include_values=True` opts into PII-carrying evidence — access-controlled sinks only); marker filter wider than the source's `"XXXX"`-only skip (`generalisation_markers=("XXXX",)` restores source-strict). One marker list drives both DPDP-1 filtering and DPDP-4 detection. Import-light: no Presidio/spaCy/FastAPI load (AC4 subprocess probe). |
...
| **Server** JWT verification | `biltiq_privacy_server.auth.verify_token(token, *, secret)` (BILTIQ-013) | The only `jwt` importer on the verify path. Verifies an HS256 token's signature + expiry against a single-element algorithm allow-list (closes alg-confusion); expired/malformed/wrong-secret/disallowed-alg all → `401` with a generic detail (never echoes token or secret). Verify-only — the request-handling runtime never calls `jwt.encode` (ADR-0007); token issuance lives only in the operator `mint` tool below. |
...
| **Server** operator token-mint | `biltiq_privacy_server.mint:main` — console-script `biltiq-privacy-mint` (BILTIQ-013a) | The **only** production `jwt.encode` caller, and a standalone operator helper — **not** in the `app:app` ASGI import graph (a structural test asserts `mint` is unreachable from `app` and that `jwt.encode` appears only here, preserving the verify-only runtime). Signs a short-lived HS256 token (`--sub`, `--ttl`, repeatable `--claim k=v`) read from `BILTIQ_JWT_SECRET` **env-only** (no `--secret` flag — keeps the key out of `argv`/`ps`); token to stdout, diagnostics to stderr. `_MINT_ALGORITHM` is pinned to `auth._ALGORITHMS[0]` by an equality test. Import graph is `argparse`+`pyjwt`+`config` (no `auth`/`fastapi`). |
