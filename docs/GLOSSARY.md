...
- **ATC-System** — Internal name for the training / fine-tuning node (address omitted from public repo).
- **GB10** — NVIDIA Grace Blackwell unified memory platform (~900 GB/s bandwidth).
- **Node 1 / 2 / 3 / 4** — The four internal GPU nodes. See `CONTEXT.md` for role mapping (addresses are not exposed publicly).
