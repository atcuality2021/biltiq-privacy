...
- 2026-06-21 — **BILTIQ-013a (PR #15 open, stacked on #14, awaiting mandatory human token-signing review):** operator token-mint CLI — `biltiq-privacy-mint` console-script (`biltiq_privacy_server.mint:main`) signs a short-lived HS256 bearer token for operator/CI smoke tests, so operators stop hand-rolling PyJWT (and stop putting the key on the command line). Verify-only invariant preserved by construction + structural test (`test_mint_invariant.py`): `jwt.encode` isolated to `mint.py`, which `app:app` never imports. Secret env-only (`BILTIQ_JWT_SECRET`, no `--secret` flag); `--sub` required, `--ttl` (default 3600, >0), repeatable `--claim k=v` (sub/iat/exp reserved). `_MINT_ALGORITHM` pinned to verify side by equality test; CLI import graph stays argparse+pyjwt+config (no auth/fastapi). ADR-0007 amended (not new — PyJWT already pinned). 22 tests, `mint.py` 100% cov (12/12 branch); full suite 90 passed; mypy --strict + ruff clean. All 8 ACs DONE. **Cannot merge until #14 merges, then rebase onto main.**
