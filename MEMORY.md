...
**Last updated:** 2026-05-18 by BILTIQ-002 reflect step
...
**Status:** Engine-port phase begun. Bootstrap (BILTIQ-000), skeleton (BILTIQ-001), memory-spine tooling (BILTIQ-003) all merged. BILTIQ-002 (8 Indian PII recognisers — patterns.py + recognisers.py) is the first engine code; PR #4 open, awaiting human reviewer. Source to port: `/home/atc/Desktop/cdcso/CDSCO-RegAI` (branch `main`).
...
- **Active sprint:** Engine port — first slice (Indian PII recognisers) in PR review.
- **Top 3 tasks in flight:**
  1. BILTIQ-002 — 8 Indian PII recognisers (Aadhaar, PAN, ABHA, GSTIN, Voter ID, IFSC, Phone, Medical Reg) via `biltiq_privacy.indian.patterns` + `biltiq_privacy.indian.recognisers` — @harish — PR #4 open, 9/9 CI green on `d060d90`, re-running on `b3d0d43` (reflect.html), awaiting reviewer.
  2. BILTIQ-004 — `age` system-binary wrapper (Python `subprocess.Popen` + `scripts/install-age.sh` for Linux/macOS/Windows). Scheduled for v0.5.0 encryption work — opened from BILTIQ-002 reflect tech-debt #1 (stack.md row currently forward-looking).
  3. Regime adapters (DPDP/GDPR/HIPAA/CCPA) — not yet ticketed; queued after BILTIQ-002 merge.
- **Top 3 risks:**
  ...
  3. age binary install path differs by OS — `scripts/install-age.sh` referenced from `stack.md` but does not yet exist. BILTIQ-004 closes this; until then the encryption row is forward-looking.
